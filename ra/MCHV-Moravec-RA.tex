...
\frame{
	...
	\vfill
	\begin{itemize}
	\item Při interakci elektromagnetického záření s hmotou může dojít k absorpci, přenosu a rozptylu.
	\item Rozptyl může být pružný a nepružný.
	\item Při pružném rozptylu nedochází k výměně energie mezi zářením a hmotou. Tento jev popsal britský fyzik Lord Rayleigh, po němž je pojmenován.\footnote[frame]{\url{http://hyperphysics.phy-astr.gsu.edu/hbase/atmos/blusky.htm\#c2}}
	\item Při nepružném rozptylu naopak k výměně energie mezi zářením a hmotou dochází. Tento jev popsal v roce 1928 Sir Chandrasekhara Venkata Raman.\footnote[frame]{\href{https://vesmir.cz/cz/casopis/archiv-casopisu/2024/cislo-4/objev-z-lodni-paluby.html}{Objev z lodní paluby}} Pojmenován byl po objeviteli \em{Ramanův efekt} nebo \em{Smekalův-Ramanův efekt}. Za tento objev obdržel sir Raman Nobelovu cenu za fyziku v roce 1930.\footnote[frame]{\url{http://www.nobelprize.org/nobel_prizes/physics/laureates/1930/raman-lecture.pdf}}
\end{itemize}
	\vfill
}
...
\frame{
	...
	\item Ramanův rozptyl je obecně slabý jev, a proto dosahuje nižší citlivosti než absorpční IR spektroskopie.
	\item Je vhodnější pro nepolární vazby a umožňuje pozorovat vibrace i na nižších vlnočtech než MIR spektroskopie ($<$400 cm$^{-1}$).\footnote[frame]{HOUSECROFT, Catherine; SHARPE, Alan. \textit{Anorganická chemie}. 1. vyd. Praha: VŠCHT, 2014. 1119 s. ISBN 978-0273-74275-3. S. 98.}
	...
	\item Pokud má molekula \emph{střed symetrie}, mohou být vibrace aktivní buď v IR nebo v RA, ale ne v obou zároveň (princip vzájemného vyloučení).
	\end{itemize}
	\vfill
}
...
\frame{
	\frametitle{}
	\vfill
	\textbf{Omezení Ramanovy spektroskopie}
	\begin{itemize}
		\item Ramanův rozptyl je velmi slabý.
		\item Intenzivní fluorescence může zcela překrýt Ramanův signál.
		\item Delší excitační vlnové délky (785 nm, 1064 nm) fluorescenci potlačují.
		\item Kompromis mezi fluorescencí a intenzitou Ramanova signálu.
	\end{itemize}

	\begin{figure}
		\adjincludegraphics[height=.5\textheight]{../img/RA-fluorescence.png}
		\caption*{Ukázka vlivu fluorescence na vzhled Ramanových spekter}
	\end{figure}
	\vfill
}
...
\node at (6.8,7.7) [text width=5cm] {Michelsonův \\ interferometr};
...
\frame{
	\frametitle{}
	\vfill
	\adjincludegraphics[height=.9\textheight]{../img/FRA106FT-RA.png}
	\vfill
}
...
\frame{
	...
	\item První funkční laser předvedl Theodore Maiman v roce 1960, teoreticky byl předpovězen (resp. stimulovaná emise) již roku 1917 Albertem Einsteinem.\footnote[frame]{\href{http://qoqi.physik.uni-erlangen.de/teaching/SS2012/Daten/Einstein.pdf}{Zur Quantentheorie der Strahlung}}
...
\frame{
	...
	\item Hustota vrypů na mřížce je řádově stovky až tisíce vrypů na milimetr.\footnote[frame]{\href{http://fyzika.jreichl.com/main.article/view/461-ohyb-svetla-na-mrizce}{Ohyb světla na mřížce}}
		...
		\begin{column}{.85\textwidth}
			\begin{itemize}
				...
				\begin{itemize}
					...
					\item CCD detektory jsou zpravidla chlazeny Peltierovým článkem, případně kapalným dusíkem.
...
\frame{
	...
	\item Umožňuje nedestruktivně měřit i spektra velkých vzorků.
...
\frame{
	...
	\item Zesílení signálu může být řádově až $10^{11}$, za optimálních podmínek byla demonstrována detekce jednotlivých molekul.\footnote[frame]{\href{https://dx.doi.org/10.1021/jp0687908}{Surface Enhanced Raman Scattering Enhancement Factors}}
...
\frame{
	...
	\begin{itemize}
		...
		\item Druhý způsob zesílení spočívá v přenosu náboje mezi povrchem a molekulou analytu. Elektronické přechody mnoha komplexů s přenosem náboje jsou ve viditelné oblasti, takže dochází k rezonančnímu zesílení. Molekuly obsahující rozsáhlé π-elektronové systémy obvykle poskytují výraznější SERS signál.
		\item První experimentální pozorování SERS bylo provedeno na pyridinu adsorbovaném na stříbře.
	\end{itemize}
...
\frame{
	...
	\begin{itemize}
		...
		\item Rozdíly v relativních intenzitách těchto pásů umožňují pomocí chemometrických metod rozlišit zdravou a nádorovou tkáň.
...
\frame{
	...
	\end{itemize}
	\begin{figure}
		\includegraphics[width=.5\textwidth]{../img/DAPI.png}
	\end{figure}
	\vfill
}
...
\frame{
	...
	\vfill
	\begin{center}
	\adjincludegraphics[height=.8\textheight]{../img/ifs28.jpg}
	\end{center}
	\vfill
}
...
\frame{
	...
	\vfill
	\begin{center}
	\adjincludegraphics[height=.8\textheight]{../img/equinox.jpg}
	\end{center}
	\vfill
}
...
\frame{
	\frametitle{}
	\vfill
	\adjincludegraphics[height=.8\textheight]{../img/alpha.jpg}
	\vfill
}
...
\frame{
	...
	\end{itemize}
	\adjincludegraphics[height=.8\textheight]{../img/raman-geo.jpg}
	\vfill
}
